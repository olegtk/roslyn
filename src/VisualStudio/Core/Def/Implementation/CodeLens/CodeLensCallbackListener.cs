...
namespace Microsoft.VisualStudio.LanguageServices.CodeLens
{
    ...
    internal class CodeLensCallbackListener : ICodeLensCallbackListener, ICodeLensContext
    {
        ...
        public async Task<ReferenceCount> GetReferenceCountAsync(CodeLensDescriptor descriptor, CodeLensDescriptorContext descriptorContext, CancellationToken cancellationToken)
        {
        ...
        public async Task<IEnumerable<ReferenceLocationDescriptor>> FindReferenceLocationsAsync(CodeLensDescriptor descriptor, CodeLensDescriptorContext descriptorContext, CancellationToken cancellationToken)
        {
            var solution = _workspace.CurrentSolution;
            if (!TryGetDocument(solution, descriptor.ProjectGuid, descriptor.FilePath, out var document))
            {
